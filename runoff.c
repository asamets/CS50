...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++)
    {
        if (strcmp(candidates[i].name, (string) name) == 0)
        {
...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        ...
        {
            if (candidates[preferences[i][n]].eliminated == false)
            {
...
int find_min(void)
{
    ...
    for (int n = 1; n < candidate_count; n++)
    {
        ...
        {
            if (candidates[n].votes < l)
            {
                l = candidates[n].votes;
            }
        }
    }
    return l;
...
bool is_tie(int min)
{
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false)
        {
            for (int n = min; n < (voter_count * candidate_count); n++)
            {
